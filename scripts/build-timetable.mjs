...
const OUT = resolve(dirname(fileURLToPath(import.meta.url)), "../api/trains/_timetable.js");
...
/* Emitted as a module, not a .json file: Vercel's bundler traces imports, and
   a JSON file reached through require() never makes it into the deployed
   function. Wrapping the text in JSON.parse keeps it as fast to load as a
   real JSON file — the engine parses JSON far quicker than a giant object
   literal — and JSON.stringify of the text does the escaping. */
const json = JSON.stringify(payload);
writeFileSync(OUT, `/* Generated by scripts/build-timetable.mjs — do not edit. */\nexport default JSON.parse(${JSON.stringify(json)});\n`);
const mb = (json.length / 1024 / 1024).toFixed(1);
console.log(`\n${trains.length} trains · ${stations.length} stations · ${schedules.length} stops`);
console.log(`written to api/trains/_timetable.js (${mb} MB)`);
