...
export default function SOSBeaconModal({ isOpen, onClose, defaultDestination = "Current Location" }) {
  ...
  const toggleSiren = () => {
    if (isSirenActive) {
      ...
      setIsSirenActive(false);
      setIsStrobeActive(false);
      toast("Siren & strobe deactivated", { icon: "🔇" });
    } else {
  ...
  const handleSendWhatsApp = () => {
    ...
    if (!selectedContactPhone) {
      toast("No emergency phone selected. Opening WhatsApp broadcast...", { icon: "ℹ️" });
    } else {
      toast.success(`Broadcasting SOS directly to ${formatWhatsAppPhoneNumber(selectedContactPhone)}...`, { icon: "📲" });
    }
      ...
      <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6 pt-16 sm:pt-20 pb-12 overflow-y-auto">
        ...
        <motion.div
          ...
          className={`fixed inset-0 bg-ink-950/90 backdrop-blur-lg transition-colors ${
            isStrobeActive ? "animate-pulse bg-red-950/80" : ""
          }`}
        />
        ...
        <motion.div
          ...
          className="relative w-full max-w-2xl bg-ink-900 border-2 border-red-500/40 rounded-3xl shadow-2xl overflow-hidden z-10 text-ivory my-auto"
        >
          {/* Header Banner */}
          <div className="relative bg-gradient-to-r from-red-600 via-rose-600 to-amber-600 p-5 sm:p-6 text-white flex items-center justify-between shadow-md">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl sm:text-2xl font-bold font-display tracking-tight flex items-center gap-2">
                  ...
                  <span className="text-xs font-mono font-normal uppercase bg-white/25 px-2 py-0.5 rounded-full">
                    24x7 Beacon
                  </span>
                </h2>
                <p className="text-xs sm:text-sm text-red-100 font-sans">
            ...
            <button
              type="button"
              onClick={onClose}
              className="w-10 h-10 rounded-2xl bg-black/40 hover:bg-black/60 text-white border border-white/30 transition-all flex items-center justify-center shadow-xl cursor-pointer hover:scale-105 active:scale-95 shrink-0"
          ...
          {isArmed && (
            <div className="bg-amber-500/15 border-b border-amber-500/30 px-5 py-3 flex items-center justify-between">
              <div className="flex items-center gap-2 text-amber-300 text-xs sm:text-sm">
                ...
                <span>
                  Broadcasting in <strong className="text-amber-200 text-base">{countdown}s</strong> — tap Cancel if accidental
                </span>
              </div>
              <button
                onClick={handleCancelCountdown}
                className="px-3 py-1 bg-amber-500 hover:bg-amber-400 text-ink-950 font-bold text-xs rounded-lg transition"
              >
                Cancel Countdown
              </button>
            </div>
          ...
          <div className="p-5 sm:p-6 max-h-[68vh] overflow-y-auto space-y-5">
            ...
            <div className="bg-ink-950/80 border border-white/10 rounded-2xl p-4 flex flex-wrap items-center justify-between gap-3 text-xs">
              <div className="flex items-start gap-2.5 flex-1 min-w-[240px]">
                ...
                <div>
                  <p className="text-sand-400 font-medium">Verified Current GPS Location</p>
                  {locationLoading ? (
                    <p className="text-sand-500 animate-pulse">Acquiring high accuracy GPS lock…</p>
                  ) : (
                    <>
                      <p className="text-ivory font-semibold text-sm">
                        {locationData?.city}, {locationData?.state}
                      </p>
                      <p className="text-sand-500 text-[11px] font-mono">
              ...
              <div className="flex items-center gap-3">
                {batteryLevel !== null && (
                  <div className="flex items-center gap-1 text-sand-300 bg-white/5 px-2.5 py-1.5 rounded-lg border border-white/5">
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-medium text-xs transition border ${
                    isSirenActive
                      ...
                      : "bg-ink-800 text-sand-300 hover:text-white border-white/10 hover:border-red-500/50"
                  }`}
                >
                  {isSirenActive ? <VolumeX className="w-4 h-4" /> : <Volume2 className="w-4 h-4 text-red-400" />}
              ...
              <div className="space-y-4">
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="block text-xs font-semibold text-sand-300 uppercase tracking-wider">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        ...
                        <button
                          ...
                          className={`text-left p-3 rounded-xl border text-xs transition flex items-center justify-between cursor-pointer ${
                            selectedContactPhone === contact.phone
                              ...
                              : "bg-ink-950/50 border-white/10 text-sand-300 hover:border-white/25"
                          }`}
                        >
                          <div>
                            ...
                            <p className="text-sand-400 text-[11px] font-mono mt-0.5">
                              {contact.phone}
                            </p>
                          </div>
                          {selectedContactPhone === contact.phone && (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sand-400 text-[11px] mt-0.5">
                  ...
                  <p className="text-[11px] text-sand-400 mt-2">
                    {selectedContactPhone ? (
                      <span className="text-emerald-300 font-medium">
                        ✓ Will dispatch directly to <span className="font-mono">{selectedContactPhone}</span>
                      </span>
                    ) : (
                      <span className="text-amber-300 font-medium">
                        ℹ️ WhatsApp will prompt you to select any contact or group from your phone book.
                      </span>
                    )}
                  </p>
                </div>
                ...
                <div>
                  <label className="block text-xs font-semibold text-sand-300 mb-1.5 uppercase tracking-wider">
                  ...
                  <input
                    ...
                    className="w-full bg-ink-950 border border-white/15 rounded-xl px-3.5 py-2.5 text-xs text-ivory placeholder-sand-600 focus:outline-none focus:border-red-500"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-emerald-600 to-green-500 hover:from-emerald-500 hover:to-green-400 text-white font-bold text-sm py-3.5 px-4 rounded-2xl flex items-center justify-center gap-2 shadow-lg shadow-emerald-900/30 transition-transform active:scale-98 cursor-pointer"
                  >
                  ...
                  >
                    {copied ? <Check className="w-4 h-4 text-emerald-400" /> : <Copy className="w-4 h-4 text-sand-300" />}
                    {copied ? "Message Copied!" : "Copy Full SOS Text"}
                  </button>
                </div>

                {/* Live Message Preview Accordion */}
                <div className="bg-ink-950/60 border border-white/10 rounded-2xl p-4 text-xs font-mono text-sand-300 space-y-1.5 whitespace-pre-line leading-relaxed">
                  <div className="text-[10px] font-sans font-semibold uppercase text-sand-500 tracking-wider mb-1 flex items-center justify-between">
                    <span>Generated Distress Payload</span>
                    <span className="text-emerald-400">Live GPS Connected</span>
                  </div>
            ...
            {activeTab === "direct_call" && (
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  <a
                    href="tel:112"
                    className="p-3.5 bg-red-950/40 hover:bg-red-900/50 border border-red-500/40 rounded-2xl flex items-center justify-between group transition"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-red-300">Police, Fire & Ambulance</p>
                      </div>
                    </div>
                  ...
                  <a
                    href="tel:1091"
                    className="p-3.5 bg-rose-950/40 hover:bg-rose-900/50 border border-rose-500/40 rounded-2xl flex items-center justify-between group transition"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-rose-300">NCW 24x7 Helpline</p>
                      </div>
                    </div>
                  ...
                  <a
                    href="tel:1363"
                    className="p-3.5 bg-amber-950/40 hover:bg-amber-900/50 border border-amber-500/40 rounded-2xl flex items-center justify-between group transition"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-amber-300">Ministry of Tourism (12 langs)</p>
                      </div>
                    </div>
                  ...
                  <a
                    href="tel:108"
                    className="p-3.5 bg-emerald-950/40 hover:bg-emerald-900/50 border border-emerald-500/40 rounded-2xl flex items-center justify-between group transition"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-emerald-300">Emergency Medical Service</p>
                      </div>
                    </div>
                ...
                <div className="bg-ink-950/70 border border-white/10 rounded-2xl p-4 text-xs space-y-1.5 mt-3">
                  <p className="text-sand-300 font-semibold flex items-center gap-1.5">
                    <Radio className="w-4 h-4 text-amber-400" />
                    {locationData?.state || "State"} Tourist Police & Nodal Assistance:
                  </p>
                  <a
                    href={getCleanTelUri(stateContacts.touristPolice)}
                    className="inline-flex items-center gap-1.5 text-amber-300 hover:text-white font-mono bg-white/5 px-2.5 py-1 rounded-lg transition"
                  >
                    <PhoneCall className="w-3 h-3 text-amber-400" />
                    <span>{stateContacts.touristPolice || "Dial 1363 (24x7 Multi-lingual)"}</span>
                  </a>
                  <p className="text-sand-500 text-[11px]">{stateContacts.stateNotes}</p>
                </div>
              </div>
            )}
            ...
            {activeTab === "contacts" && (
              <div className="space-y-4">
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-ivory uppercase tracking-wider flex items-center gap-2">
                        <span className="w-5 h-5 rounded-full bg-red-500/20 text-red-400 flex items-center justify-center text-[10px]">
                          {idx + 1}
                        </span>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        <label className="block text-[11px] text-sand-400 mb-1">Contact Name</label>
                        <input
                          type="text"
                          value={contact.name}
                          onChange={(e) => handleSaveContact(contact.id, "name", e.target.value)}
                          placeholder="e.g. Mom, Brother, Friend"
                          className="w-full bg-ink-900 border border-white/15 rounded-xl px-3 py-2 text-xs text-ivory focus:outline-none focus:border-red-500"
                        />
                      </div>
                      <div>
                        <label className="block text-[11px] text-sand-400 mb-1">WhatsApp Phone (with Country Code)</label>
                        <input
                          type="tel"
                          value={contact.phone}
                          onChange={(e) => handleSaveContact(contact.id, "phone", e.target.value)}
                          placeholder="e.g. 919876543210"
                          className="w-full bg-ink-900 border border-white/15 rounded-xl px-3 py-2 text-xs text-ivory font-mono focus:outline-none focus:border-red-500"
                        />
                      </div>
                    </div>
                  </div>
                ...
                <button
                  ...
                  className="w-full bg-red-600 hover:bg-red-500 text-white font-semibold text-xs py-2.5 rounded-xl transition"
                >
          ...
          {/* Footer */}
          <div className="bg-ink-950 border-t border-white/10 px-6 py-3.5 flex flex-wrap items-center justify-between gap-2 text-[11px] text-sand-500">
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              SafarX Emergency Core active & encrypted
            </span>
            <div className="flex items-center gap-3">
              <span className="text-sand-400 hidden sm:inline">Press ESC or</span>
              <button
                type="button"
                onClick={onClose}
                className="px-3 py-1 rounded-lg bg-white/10 hover:bg-white/20 text-white font-medium text-xs transition border border-white/10 cursor-pointer"
              >
                Close Beacon
              </button>
            </div>
