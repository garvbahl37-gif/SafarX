...
export const getSafetyMetricsForState = (stateName) => {
 ...
 const stateKey = Object.keys(STATE_SAFETY_METRICS).find((key) => key.toLowerCase() === stateName.trim().toLowerCase()
 );
 ...
 const partialKey = Object.keys(STATE_SAFETY_METRICS).find((key) => key.toLowerCase().includes(stateName.toLowerCase()) || stateName.toLowerCase().includes(key.toLowerCase())
 );
