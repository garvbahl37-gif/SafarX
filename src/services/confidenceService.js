...
export async function getSafetyScore(destination) {
 ...
 console.warn('Destination not found on Wikidata, using default safety score');
...
export function getLanguageFriendliness(destination) {
 ...
 // Default fallback
 console.warn('Language not in database, using default');
 return 70;
}
...
export async function fetchConfidenceScores(destination, travelDate, coordinates = null) {
 ...
 const overall = Math.round((safety * weights.safety) +
...
export function getTravelTips(scores) {
 ...
 tips.push('Increase safety measures - avoid isolated areas at night');
 }

 if (scores.weatherScore < 50) {
 tips.push('Pack weather-appropriate gear - challenging conditions expected');
 }
 ...
 tips.push('Learn basic phrases - limited English spoken');
 }

 if (scores.overallScore < 50) {
 tips.push('Consider alternative destinations with better conditions');
 }
