...
export function buildWhatsAppSOSPayload({
  ...
  if (address) {
    message += `🏛️ *Address / Area:* ${address}\n`;
  }
  message += `🗺️ *Google Maps Link:* ${mapsLink}\n`;
  message += `⏱️ *Time (IST):* ${timeString}\n`;
  if (batteryLevel !== null) {
    message += `🔋 *Device Battery:* ${batteryLevel}%\n`;
  }
  if (activeTripName) {
    message += `🏷️ *Active Trip:* ${activeTripName}\n`;
  }
  if (customNote) {
    message += `⚠️ *Note:* ${customNote}\n`;
  }
