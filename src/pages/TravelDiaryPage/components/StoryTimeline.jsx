...
export const StoryTimeline = ({ photos, tripTitle, summary, travelerName }) => {
 ...
 {summary && (<p className="text-ivory-muted text-sm sm:text-base leading-relaxed italic">
 "{summary}"
 </p>
 )}
 </div>
 ...
 {photos.map((photo, index) => (<motion.div
 key={photo.id || index}
 ...
 {photo.tag && (<span className="px-2.5 py-0.5 rounded-full bg-ink-800 text-ivory-muted font-medium">
 {photo.tag}
 </span>
 )}
 </div>
 <div className="flex items-center gap-3 text-ivory-muted text-[11px] font-data">
 {photo.weather && (<span className="flex items-center gap-1">
 ...
 {photo.caption && (<p className="text-ivory-muted text-sm sm:text-base italic mt-1.5 leading-relaxed">
 "{photo.caption}"
 </p>
 )}
 </div>
